...
ListaGen* tree_filter_as_list(Node* root, int (*condition)(void*), void* (*copy_info)(void*)){
    ...
    ListaGen* left = tree_filter_as_list(root->lst, condition,copy_info);

    ListaGen* right = tree_filter_as_list(root->rst, condition, copy_info);
    
    if(condition(root->info))
        right = lstgen_insere(right, copy_info(root->info));

    return lstgen_conc(left, right);
}
...
Node* tree_delete_node(Node* root, int (*condition)(void*)){
    ...
    if(condition(root->info)){
        ...
        Node* min = tree_find_min(root->rst);
        root->info = min->info;
...
int tree_get_height(Node* root){
    ...
    int height_l = tree_get_height(root->lst);
    int height_r = tree_get_height(root->rst);
...
int tree_how_many_nodes(Node* root){
    ...
    int nodes_l = tree_how_many_nodes(root->lst);
    int nodes_r = tree_how_many_nodes(root->rst);
...
int tree_how_many_leaves(Node* root){
    ...
    if(root->lst == NULL && root->rst == NULL)
        count = 1;
    return count + tree_how_many_leaves(root->lst) + tree_how_many_leaves(root->rst);
}
...
int tree_get_level(Node* root, void* info, int (*compare)(void*, void*)){
    if(tree_empty(root)) return -1;
    if(compare(root->info, info)) return 0;

    int level_l = tree_get_level(root->lst, info, compare);
    if(level_l != -1) return level_l + 1;

    int level_r = tree_get_level(root->rst, info, compare);
    if(level_r != -1) return level_r + 1;
